(() => {
  ...
  safeModeLabel.textContent = "🛡️ Safe Mode";
  ...
  // Turbo Mode Toggle
  const turboModeWrapper = document.createElement("div");
  turboModeWrapper.style.cssText = `
    display: flex;
    align-items: center;
    gap: 10px;
    background: rgba(255,255,255,0.1);
  ...
  modeGroup.appendChild(safeModeWrapper);
  modeGroup.appendChild(turboModeWrapper);
  ...
  const delay = (ms) => {
    ...
    const turboMode = document.getElementById("turboModeToggle")?.checked;
    
    let base = 500; // Default base 500ms (changed from variable)
    let randomExtra = Math.floor(Math.random() * 300); // Random 0-300ms (500-800ms total)
    
    if (safeMode) {
      base = 800;
      randomExtra = Math.floor(Math.random() * 400);
    } else if (turboMode) {
      base = 55;
      randomExtra = 0;
    }
  ...
  const runDeleteProcess = async (mode = "all") => {
    ...
    updateDescription("Sedang memproses penghapusan...");
  ...
  turboModeCheckbox.addEventListener('change', function() {
    if(this.checked) {
      ...
      updateStatus("🚀 Turbo Mode aktif: kecepatan maksimal!");
    } else {
      updateStatus("⚡ Mode standar aktif.");
    }
  });
